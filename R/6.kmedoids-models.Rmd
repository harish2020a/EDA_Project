...
model<- ctree(as.factor(class) ~ ., train)
plot(model)
```

```{r}
prediction <- predict(model, test[-length(test)])
...
#conclusion that Random Forest performs the best with ~93.50% accuracy
```
...
#Voting Classifier Increases the accuracy from ~81.96% to ~88.52%
```
